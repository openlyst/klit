...
class InlineVideoTimeline extends StatefulWidget {
  const InlineVideoTimeline({super.key, required this.player});

  final VideoPlayer player;

  @override
  State<InlineVideoTimeline> createState() => _InlineVideoTimelineState();
}

class _InlineVideoTimelineState extends State<InlineVideoTimeline> {
  bool seeking = false;
  Duration position = Duration.zero;
  Duration duration = Duration.zero;
  Duration buffer = Duration.zero;
  ...
  Widget build(BuildContext context) {
    ...
    final hasDuration = maxMillis > 0;
    final clampedPosition = hasDuration
        ? position.inMilliseconds
            .toDouble()
            .clamp(0.0, maxMillis)
        : 0.0;
    final clampedBuffer = hasDuration
          ...
          child: LayoutBuilder(
            builder: (context, constraints) {
              ...
              if (!w.isFinite || !h.isFinite || w <= 0 || h <= 0) {
                ...
                final maxH = size.height.isFinite ? size.height : 1080.0;
